...
class KernelPerformanceInfo {
 public:
  KernelPerformanceInfo(std::string kName, KernelExecutionType kernelType,
                        ldms_t* the_ldms, const char* node_name,
                        const int rank_no, const int job_id,
                        const double job_start, const uint64_t job_epoch_start,
                        const uint16_t kernel_nest_level,
                        const int tool_verbosity, bool* ldms_global_publish)
      : kType(kernelType),
        ldms(the_ldms),
        nodename(node_name),
        rank(rank_no),
  ...
  void addFromTimer() {
    ...
    if ((*ldms_publish)) {
      ...
      snprintf(big_buffer, buffer_size,
               "{ \"job-id\" : %d, \"node-name\" : \"%s\", \"rank\" : %d, "
               "\"timestamp\" : \"%.6f\", \"kokkos-perf-data\" : [ { \"name\" "
               ": \"%s\", \"type\" : %d, \"current-kernel-count\" : %llu, "
               "\"total-kernel-count\" : %llu, \"level\" : %u, "
               "\"current-kernel-time\" : %.9f, \"total-kernel-time\" : %.9f } "
               "] }\n",
               jobid, nodename, rank, epoch_stamp,
               (NULL == kernelName) ? "" : kernelName, (int)kType, callCount,
               kernel_ex * kernelSampleRate, nestingLevel, sample_time,
               total_time);
      ...
      // int rc = ldmsd_stream_publish( (*ldms), "kokkos-perf-data",
      // LDMSD_STREAM_JSON, big_buffer, strlen(big_buffer) + 1);
